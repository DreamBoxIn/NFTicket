...
export function PaymentModal({ ticket, event, quantity, onClose }: PaymentModalProps) {
  ...
  const gasFee = ticket.currency === "BTC" ? 0.0001 : 5 // Adjust gas fee based on currency
              ...
              <div className="p-4 bg-muted rounded-lg">
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    <span>{ticket.type} {t("common.ticket")} × {quantity}</span>
                    <span>{total} {ticket.currency}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>{t("tickets.gasFee")}</span>
                    <span>{gasFee} {ticket.currency}</span>
                  </div>
                  <div className="pt-2 border-t flex justify-between font-bold">
                    <span>{t("tickets.total")}</span>
                    <span>{(total + gasFee).toFixed(ticket.currency === "BTC" ? 8 : 2)} {ticket.currency}</span>
                  </div>
                </div>
              </div>
              <Button 
